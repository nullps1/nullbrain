# NullCode 0.1: local inference queue
...
Extract into `/srv/nullbrain/src`, then:
...
curl --fail --silent --show-error http://127.0.0.1:8080/jobs
sudo docker compose logs --tail 50 controller
```
...
The image has no host Docker socket, repository mounts, GitHub credentials, or model-controlled command execution. Future increments add isolated Java builds and repository operations separately.
