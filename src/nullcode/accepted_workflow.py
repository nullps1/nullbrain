...
STREAM_LOOP_GUIDANCE = (
    "IMPLEMENTATION GUIDANCE: Replace the invalid array.stream() chain with an "
    "ordinary for loop over the split words. Do not use Streams. "
    "Retain all contract requirements and existing behavior.\n"
)


def compiler_repair_rule(result, target):
    """Match one javac error block, not model prose or unrelated JUnit output."""
    compiled = result.get("compile") or {}
    if (result.get("passed") or not result.get("repairable")
            or compiled.get("exit_code") != 1 or compiled.get("timed_out")
            or result.get("cleanup", {}).get("exit_code") != 0
            or result.get("cleanup", {}).get("timed_out")):
        return None
    lines = str(compiled.get("log") or "").splitlines()
    header = re.compile(r"^\s*(.+\.java):\d+: error: (.+)$")
    for index, line in enumerate(lines):
        match = header.match(line)
        if not match or match.group(2).strip() != "cannot find symbol":
            continue
        ...
        evidence = "\n".join(block)
        if (re.search(r"(?m)^\s*symbol:\s+method\s+stream\(\)\s*$", evidence)
                and re.search(r"(?m)^\s*location:\s+class\s+(?:java\.lang\.)?String\[\]\s*$", evidence)):
            return {"id": "javac-string-array-stream-loop-v1",
                    "guidance": STREAM_LOOP_GUIDANCE,
                    "evidence": line + "\n" + evidence}
    return None
...
def prompt_for(contract, source, diagnostic=None, guidance=None):
    text = (
    ...
    if guidance is not None:
        if diagnostic is None:
            raise ValueError("Compiler guidance requires a repair diagnostic")
        text = guidance + text
...
def run_job(store, job, generate_fn=generate, verify_fn=verify, artifacts=JOBS):
    ...
    try:
        ...
        for number in (1, 2, 3):
            ...
            applied_rule = repair_rule
            if applied_rule:
                save(folder, "repair-rule.json", applied_rule)
            prompt = prompt_for(contract, source, diagnostic,
                                guidance=applied_rule["guidance"] if applied_rule else None)
            ...
            try:
                ...
                repair_rule = compiler_repair_rule(result, target)
            result["applied_repair_rule"] = applied_rule["id"] if applied_rule else None
            history.append({"attempt": number, "passed": result["passed"],
                            "repair_rule": result["applied_repair_rule"]})
